...
| `work abort [--force]` | `active`, possédé par la session ; rien à perdre (aucun commit au-delà de la base, aucun fichier suivi modifié) sauf `--force` | Abandonne le ticket : retour sur main, suppression de la branche **locale** (jamais la distante), état `free` (les `pending_prs` sont conservées). Sans `--force`, refuse et renvoie vers `work park` en listant ce qui serait perdu. Ajoutée le 2026-09-16 : démarrer un ticket puis l'abandonner n'avait aucune sortie propre. |
